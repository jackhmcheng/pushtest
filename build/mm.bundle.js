...
.factory('$mmFilepool', ["$q", "$log", "$timeout", "$mmApp", "$mmFS", "$mmWS", "$mmSitesManager", "$mmEvents", "md5", "mmFilepoolStore", "mmFilepoolLinksStore", "mmFilepoolQueueStore", "mmFilepoolFolder", "mmFilepoolQueueProcessInterval", "mmCoreEventQueueEmpty", "mmCoreDownloaded", "mmCoreDownloading", "mmCoreNotDownloaded", "mmCoreOutdated", "mmCoreNotDownloadable", "mmFilepoolPackagesStore", "mmCoreEventPackageStatusChanged", "$mmText", "$mmUtil", "mmFilepoolWifiDownloadThreshold", "mmFilepoolDownloadThreshold", function($q, $log, $timeout, $mmApp, $mmFS, $mmWS, $mmSitesManager, $mmEvents, md5, mmFilepoolStore,
    ...
    self.FILEOUTDATED = 'outdated';
        ...
        $log.debug('Set statusZZZ \'' + status + '\' for package ' + component + ' ' + componentId);
        revision = revision || 0;
        timemodified = timemodified || 0;
        return $mmSitesManager.getSite(siteId).then(function(site) {
            var db = site.getDb(),
...
.factory('$mmSitesManager', ["$http", "$q", "$mmSitesFactory", "md5", "$mmLang", "$mmApp", "$mmUtil", "$mmEvents", "$state", "$translate", "mmCoreSitesStore", "mmCoreCurrentSiteStore", "mmCoreEventLogin", "mmCoreEventLogout", "$log", "mmCoreWSPrefix", "mmCoreEventSiteUpdated", "mmCoreEventSiteAdded", "mmCoreEventSessionExpired", "mmCoreEventSiteDeleted", "$mmText", "mmCoreConfigConstants", function($http, $q, $mmSitesFactory, md5, $mmLang, $mmApp, $mmUtil, $mmEvents, $state,
	...
	self.userRegister = function(siteurl, username, password, firstname, lastname, token, service, retry) {
        retry = retry || false;
        if (!$mmApp.isOnline()) {
            return $mmLang.translateAndReject('mm.core.networkerrormsg');
        }
        ...
        var registerURL = siteurl + 'webservice/rest/server.php?wstoken=' + token + '&wsfunction=core_user_create_users&moodlewsrestformat=json';
		 var user = {
		...
		*/
		...
		return $http.post(registerURL, user, config).then(function(response) {
            var data = response.data;
            if (typeof data == 'undefined') {
                return $mmLang.translateAndReject('mm.core.cannotconnect');
            } else {
				...
				if (typeof data.errorcode != 'undefined') {
					return $mmLang.translateAndReject('The email address is duplicate');
				}			
				var result = data[0];				
				return result;
				/*
...
angular.module('mm.core')
.directive('mmFile', ["$q", "$log", "$mmUtil", "$mmFilepool", "$mmSite", "$mmApp", "$mmEvents", "$mmFS", "mmCoreDownloaded", "mmCoreDownloading", "mmCoreNotDownloaded", "mmCoreOutdated", function($q, $log, $mmUtil, $mmFilepool, $mmSite, $mmApp, $mmEvents, $mmFS, mmCoreDownloaded, mmCoreDownloading,
...
.config(["$stateProvider", "$urlRouterProvider", "$mmInitDelegateProvider", "mmInitDelegateMaxAddonPriority", function($stateProvider, $urlRouterProvider, $mmInitDelegateProvider, mmInitDelegateMaxAddonPriority) {
    ...
    .state('mm_login.credentials', {
        ...
        onEnter: ["$state", "$stateParams", function($state, $stateParams) {
			$stateParams.siteurl = "http://www.medootest.com/moodle/";
            if (!$stateParams.siteurl) {
              $state.go('mm_login.init');
            }
        }]
    })
    .state('mm_login.registration', {
        url: '/registration',
        templateUrl: 'core/components/login/templates/registration.html',
        controller: 'mmRegistrationCtrl',
...
.controller('mmLoginCredentialsCtrl', ["$scope", "$state", "$stateParams", "$mmSitesManager", "$mmUtil", "$ionicHistory", "$mmApp", "$q", "$mmLoginHelper", "$translate", "$mmContentLinksDelegate", "$mmContentLinksHelper", function($scope, $state, $stateParams, $mmSitesManager, $mmUtil, $ionicHistory, $mmApp,
    ...
    $scope.goToRegister = function() {
        return $state.go('mm_login.registration');
        /*
                var siteurl = "http://www.medootest.com/moodle/";
                    username = $scope.credentials.username,
                    password = $scope.credentials.password;

                //var response = $mmSitesManager.getUserToken(siteurl, "admin", "Pass1234#");
                return $mmSitesManager.getUserToken(siteurl, "webuser1", "webuser1").then(function(data) {
                    return $mmSitesManager.userRegister(siteurl, username, password, "794b6cb0c974006f49e72c435ec6dcde").then(function(result) {
                      if (result && result.warning) {
                        $mmUtil.showErrorModal(result.warning, true, 4000);
                      }
                    });
...
angular.module('mm.core.login')
    .controller('mmRegistrationCtrl', ["$scope", "$state", "$stateParams", "$mmSitesManager", "$mmUtil", "$ionicHistory", "$mmApp", "$q", "$mmLoginHelper", "$translate", "$mmContentLinksDelegate", "$mmContentLinksHelper", function($scope, $state, $stateParams, $mmSitesManager, $mmUtil, $ionicHistory, $mmApp,
               $q, $mmLoginHelper, $translate, $mmContentLinksDelegate, $mmContentLinksHelper) {
        $scope.siteurl = $stateParams.siteurl;
        $scope.credentials = {
            username: $stateParams.username
        ...
        $scope.login = function() {
            ...
            if (!siteChecked) {
                return checkSite(siteurl).then(function() {
                    if (!$scope.isBrowserSSO) {$mmSitesManager.getUserToken
                        return $scope.login();
                    }
                });
            } else if ($scope.isBrowserSSO) {
            ...
            var modal = $mmUtil.showModalLoading();
            return (siteurl, username, password).then(function(data) {
                return $mmSitesManager.newSite(data.siteurl, data.token).then(function() {
                    delete $scope.credentials;
                    $ionicHistory.nextViewOptions({disableBack: true});
                    if (urlToOpen) {
        ...
        $scope.submitRegister = function() {
            ...
            if (!username) {
                $mmUtil.showErrorModal('email address required', true);
                return;
            }
			else {
				var isMatchRegex = EMAIL_REGEXP.test(username);
				if (!isMatchRegex){
					$mmUtil.showErrorModal('email address is invalid', true);
					return;
				}
			}
            if (!password) {
				...
				if (password.length < 6){
					$mmUtil.showErrorModal('The minimum lenght of password is 6', true);
					return;
				}
			}
			
            if (!firstname) {
                $mmUtil.showErrorModal('First Name requried', true);
                return;
            }
            if (!lastname) {
                $mmUtil.showErrorModal('Last Name required', true);
                return;
            }
            var modal = $mmUtil.showModalLoading();
            return $mmSitesManager.getUserToken(siteurl, "webuser1", "webuser1").then(function(data) {
				var userToken = data.token;
				return $mmSitesManager.userRegister(siteurl, username, password, firstname, lastname, "221dde1e5b2e104b8ad29a3550c3b4b9").then(function(result) {
					if (result && result.warning) {
						$mmUtil.showErrorModal(result.warning, true, 4000);
                    }
					else {
...
.constant('mmCoreConfigConstants', {
    ...
    "gcmpn": "580262980937",//"694767596569",
